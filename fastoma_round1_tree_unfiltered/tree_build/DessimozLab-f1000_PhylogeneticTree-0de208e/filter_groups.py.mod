...
def convert_fasta_file(infile, outfile, min_nr_species):
    ...
    seen = set([])
    # Open compressed or uncompressed automatically
    open_func = gzip.open if infile.endswith(".gz") else open

    with open_func(infile, "rt") as handle:
        records = [convert_id_to_species_name(z) for z in Bio.SeqIO.parse(handle, 'fasta')]
